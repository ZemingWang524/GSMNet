...
from models.gsmnet import GSMNet
...
def train_pyg(
    ...
    # For some low-data targets we intentionally use a 90/10 train-test split (no validation split).
    has_val = len(val_loader.dataset) > 0 and len(val_loader) > 0
    ...
    def log_results(engine):
        ...
        if has_val:
            ...
            vmetrics = {m: float("nan") for m in metrics.keys()}
    ...
    if config.n_early_stopping is not None and has_val:
    ...
    if config.log_tensorboard:
        ...
        eval_pairs = [("training", train_evaluator)]
        if has_val:
            eval_pairs.append(("validation", evaluator))
        for tag, evaluator in eval_pairs:
    ...
    with torch.no_grad():
        ...
        for dat, id in tqdm(zip(test_loader, ids)):
            ...
            # pred, features = net(data.to(device))
            # out_data = pred.cpu().numpy().tolist()
            ...
            # out_data = pred.cpu().numpy().tolist()
            # # ...existing code...
            # tsne_features.append(features.cpu().numpy())
    ...
    # tsne_save_path = os.path.join(
    #     config.output_dir,
    #     f"tsne_{config.model.name}_{config.target}_features.npy"
    # )
    # np.save(tsne_save_path, tsne_features)
    # print(f"t-SNE features saved to {tsne_save_path}")
...
def train_prop_model(config: Dict, data_root: str = None, checkpoint: str = None, testing: bool = False, file_format: str = 'poscar'):
    ...
    # Reviewer-requested setting for specific JARVIS targets:
    # split into two portions, 90% for training and 10% for testing.
    if config.get("target") in special_split_targets:
