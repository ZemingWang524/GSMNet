...
class StructureDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        # print(f"Processing test sample {i}/{len(mat_data)}")
        ...
        for i in tqdm(range(len(mat_data))):
            ...
            if hasattr(mat_data[i], "edge_index"):
                ei = mat_data[i].edge_index
            elif hasattr(mat_data[i], "inf_edge_index"):
                ei = mat_data[i].inf_edge_index
            else:
                ei = torch.empty((2, 0), dtype=torch.long)

            n = mat_data[i].x.size(0)
            if ei.numel() > 0:
                src, dst = ei[0], ei[1]
            ...
            inv[order] = torch.arange(n, dtype=torch.long)

            mat_data[i].mamba_order = order
            mat_data[i].mamba_inv = inv
...
def load_infinite_graphs(
    ...
    def atoms_to_graph(atoms):
        ...
        edge_u, edge_v, edge_r = build_undirected_edgedata(atoms=structure, edges=edges)
        edge_index_local = torch.stack([edge_u, edge_v])
        
        edge_nei = []
        for i in range(edge_index_local.size(1)):
            center_node = edge_index_local[0, i]
            mask = edge_index_local[0] == center_node
            mask[i] = False
            ...
            for idx in neighbor_indices:
                nei_vec = edge_r[idx]
                nei_vectors.append(nei_vec)
            while len(nei_vectors) < 3:
                nei_vectors.append(torch.zeros_like(edge_r[0]))
            edge_nei.append(torch.stack(nei_vectors))
        if edge_index_local.size(1) > 0:
            edge_nei = torch.stack(edge_nei)
        else:
            edge_nei = torch.zeros((0, 3, 3))

        data = Data(
            x=node_features, 
            # edge_attr=edge_r.norm(dim=-1), 
            edge_attr=edge_r,  
...
def get_train_val_loaders(
    ...
    # Some targets intentionally use a 90/10 train-test split (n_val=0).
    # In that case, skip validation dataset construction to avoid
    # pandas KeyError on an empty dataframe.
    if len(dataset_val) == 0:
        print("validation split is empty (n_val=0), skip val dataset processing.")
        val_data = torch.utils.data.Subset(train_data, [])
    else:
